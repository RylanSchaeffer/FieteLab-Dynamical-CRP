...
def plot_dinari_covertype_labels_by_obs_idx(labels: np.ndarray,
                                            plot_dir: str,
                                            title_str: str = None):

    plt.close()
    index = 1 + np.arange(len(labels))
    sns.scatterplot(x=index, y=labels, s=3)
    plt.xlabel('Obs Index')
    plt.ylabel('Cover Type (Class Label)')
    ...
    plt.savefig(os.path.join(plot_dir,
                             f'dinari_covertype_labels_by_obs_idx.png'),
                bbox_inches='tight',
                dpi=300)
    # plt.show()
    plt.close()
