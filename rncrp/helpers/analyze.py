    ...
    for dynamics_str, sweep_subset_results_df in all_inf_algs_results_df.groupby('dynamics_str'):
        ...
        for param_tuple, df_by_n_features in sweep_subset_results_df.groupby(['alpha',
            ...
            if param_tuple[-1] == 'Dynamical-CRP':
                ...
                concatenated_inferred_to_observed_true_df = pd.concat(all_inferred_to_observed_true_data_array, ignore_index=True)
                concatenated_inferred_to_total_true_df = pd.concat(all_inferred_to_total_true_data_array, ignore_index=True)
                concatenated_observed_true_to_total_true_df = pd.concat(all_observed_true_to_total_true_data_array, ignore_index=True)

                # Save each dataframe
                concatenated_inferred_to_observed_true_df.to_pickle(cluster_ratio_plot_dir+'/concatenated_inferred_to_observed_true_df.pkl')
                concatenated_inferred_to_total_true_df.to_pickle(cluster_ratio_plot_dir+'/concatenated_inferred_to_total_true_df.pkl')
                concatenated_observed_true_to_total_true_df.to_pickle(cluster_ratio_plot_dir+'/concatenated_observed_true_to_total_true_df.pkl')

                # Store paths to the concatenated dataframes
                inferred_to_observed_true_data_paths_array.append(cluster_ratio_plot_dir+'/concatenated_inferred_to_observed_true_df.pkl')
                inferred_to_total_true_data_paths_array.append(cluster_ratio_plot_dir+'/concatenated_inferred_to_total_true_df.pkl')
                observed_true_to_total_true_data_paths_array.append(cluster_ratio_plot_dir+'/concatenated_observed_true_to_total_true_df.pkl')
                # count += 1

        # TODO: Check that specified directory works
        np.save(os.path.join(sweep_dynamics_str_dir, 'inferred_to_observed_true_data_paths_array.npy'),np.array(inferred_to_observed_true_data_paths_array))
        np.save(os.path.join(sweep_dynamics_str_dir, 'inferred_to_true_true_data_paths_array.npy'),np.array(inferred_to_total_true_data_paths_array))
        np.save(os.path.join(sweep_dynamics_str_dir, 'observed_true_to_total_true_data_paths_array.npy'), np.array(observed_true_to_total_true_data_paths_array))
